                ...
                >
                  <div className="flex items-center justify-between p-5 rounded-2xl border-2 border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 bg-white dark:bg-zinc-900 hover:shadow-lg transition-all duration-200 group-hover:scale-[1.01]">
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="relative">
                        <div className={`
                          ...
                          ${monitor.paused ? 'bg-zinc-100 dark:bg-zinc-800' : ''}
                          ${!monitor.paused && monitor.status === 'HEALTHY' ? 'bg-green-100 dark:bg-green-950' : ''}
                          ${!monitor.paused && monitor.status === 'LATE' ? 'bg-yellow-100 dark:bg-yellow-950' : ''}
                          ${!monitor.paused && monitor.status === 'FAILED' ? 'bg-red-100 dark:bg-red-950' : ''}
                        `}>
                          {monitor.paused ? (
                            <svg className="w-6 h-6 text-zinc-600 dark:text-zinc-400" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                    ...
                    <div>
                      <Badge 
                        variant={
                          monitor.paused ? 'default' :
                          monitor.status === 'HEALTHY' ? 'success' :
                          monitor.status === 'LATE' ? 'warning' :
                          'danger'
                        }
                        size="md"
                        pulse={!monitor.paused && monitor.status !== 'HEALTHY'}
                      >
